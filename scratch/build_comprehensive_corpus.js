...
morningRaw.forEach((item, idx) => {
  ...
  // If text contains evening variations, convert to morning for morning category
...
morningRaw.forEach((item, idx) => {
  let text = cleanString(item.text);
  // If text contains morning variations, convert to evening for evening category
...
raw.forEach((catObj, catIdx) => {
  if (catIdx === 0) return; // already processed for morning & evening
...
console.log('Total generated items:', allItems.length);
console.log('Counts per category:');
const catSummary = {};
allItems.forEach(i => catSummary[i.category] = (catSummary[i.category] || 0) + 1);
console.log(catSummary);

// Verify NO EMOJIS
let hasEmoji = false;
allItems.forEach(i => {
  if (EMOJI_REGEX.test(i.text) || EMOJI_REGEX.test(i.fadl) || EMOJI_REGEX.test(i.source) || EMOJI_REGEX.test(i.categoryName)) {
    hasEmoji = true;
  }
});
