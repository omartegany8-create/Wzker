...
class WzkerAudioEngine {
  ...
  initEvents() {
    ...
    this.audio.addEventListener('playing', () => {
      if (this.isRadioActive()) {
        this.updateRadioTelemetry('متصل الآن (بث نقي)', '#4ade80');
    ...
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      setTimeout(() => {
        if (this.isRadioActive()) {
          ...
          this.audio.play().catch(e => console.log('Reconnect retry:', e));
        }
      }, 1500);
    } else {
  ...
  _startAudioPlayback(track) {
    ...
    if (playPromise !== undefined) {
      playPromise.then(() => {
        ...
        if (window.wzkerLib) window.wzkerLib.addToHistory(track);
        if (window.wzkerQuranSync) window.wzkerQuranSync.onTrackChanged(track);
      }).catch(err => {
        console.log('Playback start handled:', err);
        this.updateUI();
      });
    }
  }
  ...
  seek(percent) {
    ...
    if (miniProg) miniProg.style.width = clamped + '%';
    if (fpProg) fpProg.style.width = clamped + '%';
    if (curTimeEl) curTimeEl.textContent = this.formatTime(targetTime);

    // Keep smooth continuous playback
